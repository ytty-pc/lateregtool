...
Step 3  設定一覧
        ...
        プリセット5種（初心者 / 安全重視 / セキュリティ優先 / 競合なし / 全部盛り）も利用可能
        → サイドバーから .bat をダウンロード
...
| 🤝 競合なし | 競合9ペアを全回避・impact:high優先で選択 | 競合ペアの低インパクト側を除外（約20枚） |
...
40bit（5バイト）のビットマスク方式。`SHARE_IDS` 配列で固定された28枚のID順序でビットを割り当て。環境情報（GPU/OS/ジャンル）もビットに含む。URLパラメータ `?lt=LT3_xxx` でそのまま共有可能。

```
bit 0    : 未使用（将来拡張用）
bit 1–28 : 各カードのチェック状態（SHARE_IDS順）
bit 29–31: 環境情報（GPU/OS/ジャンル）
```
